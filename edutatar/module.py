...
class EduTatarModule(types.BaseModule):
    ...
    async def set_language_from_callback(self, update: types.CallbackQuery):
        language = ""
        if update.data == "set_russian":
            ...
            await self.bot.edit_message_text(chat_id=update.message.chat.id, message_id=update.message.message_id,
    ...
    async def main_menu_message(self, update: types.Message):
    ...
    async def show_daily_grades(self, user_id: int, language: str, update: types.CallbackQuery):
        ...
        if login_and_password is None:
            ...
            return
        result, DNSID, dates1, dates2 = await self.parser.getDay(login_and_password[0], login_and_password[1],
    ...
    async def update_quarter_grades(self, user_id: int, language: str, update: types.CallbackQuery):
        login_and_password = await self.db.get_login_and_password(user_id)
        if login_and_password is None:
            await self.bot.send_message(chat_id=update.from_user.id,
                                   text="Something went wrong. We apologise.")  # TODO: do not do like that hehe
        ...
        rounding_rule = await self.db.get_rounding_rule(user_id)
        result, DNSID = await self.parser.getTerm(login_and_password[0], login_and_password[1],
                                             termNum=update.data, DNSID=self.db.get_value(user_id, "DNSID"), rounding_rule=rounding_rule)
        if DNSID is not None:
            self.db.set_value(user_id, "DNSID", DNSID)
        await self.bot.edit_message_text(chat_id=user_id, message_id=update.message.message_id, text=result,
                                    reply_markup={
                                        "inline_keyboard": [[
                                            {
                                                "text": get_text(language, "first_quarter"),
    ...
    async def show_year_grades(self, user_id: int, language: str, update: types.CallbackQuery):
        login_and_password = await self.db.get_login_and_password(user_id)
        if login_and_password is None:
            await self.bot.send_message(chat_id=update.from_user.id,
                                   text="Something went wrong. We apologise.")  # TODO: upper to-do
            return
        result, DNSID = await self.parser.getYear(login_and_password[0], login_and_password[1],
    ...
    async def update_language_and_message_settings(self, user_id: int, language: str, update: types.CallbackQuery):
    ...
    async def go_back_callback(self, user_id: int, language: str, update: types.CallbackQuery):
        user_id = update.from_user.id
        language = await self.db.get_language(user_id)
    ...
    async def send_message_change_rounding(self, user_id: int, language: str, update: types.CallbackQuery):
        await self.bot.send_message(user_id, get_text(language, "enter_rounding"))
        self.db.set_value(user_id, "state", "enter_rounding")

    async def change_daily_grades(self, user_id: int, language: str, update: types.CallbackQuery):
        login_and_password = await self.db.get_login_and_password(user_id)
        if login_and_password is None:
            await self.bot.send_message(chat_id=update.from_user.id,
                                   text="Something went wrong. We apologise.")  # TODO: do not do like that hehe
        ...
        if DNSID is not None:
            self.db.set_value(user_id, "DNSID", DNSID)
        await self.bot.edit_message_text(message_id=update.message.message_id, chat_id=update.message.chat.id, text=result,
                                    reply_markup={
                                        "inline_keyboard": [[
                                            {
                                                "text": "<--",
    ...
    async def settings_menu(self, update: types.CallbackQuery):
        user_id = update.from_user.id
    ...
    async def send_message_choose_language(self, update: types.Message):
        message = await self.bot.send_message(update.chat.id, "✋ Здравствуйте, выберите свой язык:",
                                         reply_markup={
                                             "inline_keyboard": [[
                                                 {
                                                     "text": "Русский 🇷🇺",
    ...
    async def check_login_and_password(self, user_id: int):
        ...
        DNSID = await self.parser.get_DNSID(
            login=login,
            password=password
        )
        if DNSID is None:
    ...
    async def onCallbackQuery(self, update: types.CallbackQuery):
        user_id = update.from_user.id
        language = await self.db.get_language(user_id)
    ...
    async def onMessageOnly(self, update: types.Message):
        user_id = update.from_user.id
        ...
        if state is None:
            ...
            if not update.text.isnumeric():
                await self.bot.send_message(chat_id=user_id, text=get_text(language, "incorrect_login_and_password"))
                return
            self.db.set_value(user_id, "login", update.text)
            self.db.set_value(user_id, "state", "password")
            await self.bot.send_message(chat_id=user_id, text=get_text(language, "enter_password"))
        elif state == "password":
            self.db.set_value(user_id, "password", update.text)
            await self.bot.send_message(chat_id=user_id, text=get_text(language, "let_me_check_login_and_password"))
            result_checking = await self.check_login_and_password(user_id)
            if not result_checking:
                await self.bot.send_message(chat_id=user_id, text=get_text(language, "incorrect_login_and_password"))
                self.db.set_value(user_id, "state", "login")
            else:
                await self.db.set_login_and_password(user_id, self.db.get_value(user_id, "login"), self.db.get_value(user_id, "password"))
                self.db.set_value(user_id, "state", "main")
                await self.bot.send_message(user_id, get_text(language, "thanks"), reply_markup={
                    'keyboard': [["Меню"]],
            ...
            if not update.text.isnumeric() or len(update.text) != 2 or not (10 <= int(update.text) <= 99):
                ...
                await self.bot.send_message(user_id, get_text(language, "successfully_changed"))
                self.db.set_value(user_id, "state", "main")
                await self.main_menu_message(update)
        elif state == "main":
            await self.main_menu_message(update)
    ...
    def get_funcs(self):
        return [
            [self.onMessageOnly, types.Handlers.onMessageOnly],
            [self.onCallbackQuery, types.Handlers.onCallbackQuery]
        ]